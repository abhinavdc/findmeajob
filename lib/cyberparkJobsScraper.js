      ...
      getCyberparkJobs().then((fetchedEntries) => {
        jobsCollection
          ...
          .then((data) => {
            ...
            //
            // remove all entries from db which is earlier than lastExistingEntry
            //

            // get all jobs in site
            const allIds = fetchedEntries.map((x) => x.jobId);

            console.log('new ids', allIds);

            // filter out jobs before last fetch
            const newIdsFetchedPrev = allIds.filter(
              (x) => x <= lastExistingEntry
            );
            console.log('old ids', newIdsFetchedPrev);

            // find jobs removed from site
            const removedEntries = existingEntries.filter(
              (x) => !newIdsFetchedPrev.includes(x.jobId)
            );

            const removedEntryIds = removedEntries.map((x) => x.jobId);
            console.log('not existing ids in site', removedEntryIds);

            // remove enteries in db which does not exist in site
            jobsCollection.deleteMany({
              parkId: 3,
              jobId: { $in: removedEntryIds },
            });
